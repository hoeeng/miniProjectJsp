...
<c:choose>
	...
	<c:when test="${param.num != null}">
		<c:set var="result" value="${likeyDAO.boardLikeUp(param.num, sessionScope.loginSuccess) }"/>
		<script>
	...
	<c:when test="${param.datnum != null }">
		<c:set var="datresult" value="${likeyDAO.datLikeUp(param.datnum, sessionScope.loginSuccess)}" />
		<script>
			location.href="contentView.jsp?title=${param.goback}"
		</script>
	</c:when>
